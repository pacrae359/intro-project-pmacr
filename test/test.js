...
const { addPerson, deletePerson, getPersonViaName } = require ( "../lib/db.js" )
const { expect } = chai
...
describe( "Person Modules", () => {
  ...
  it( "should stop duplicate emails when updating people", async () => {
    try {
      const personrow = await getPersonViaName( "TestName" )
      const thisPerson = personrow[ 0 ]
      thisPerson.notes = "Updated Notes!"
      await peopleModule.edit( thisPerson.id, thisPerson.name , thisPerson.email, thisPerson.notes )
    } catch ( err ) {
  ...
  it( "should update a person with a unique email", async () => {
    ...
    await peopleModule.edit( {}, "PUT", thisPerson.id, thisPerson.name , thisPerson.email, thisPerson.notes )
    const newPersonRow = await getPersonViaName( "TestName" )
    const checkPerson = newPersonRow[ 0 ]
...
describe( "API Testing", () => {
  ...
  it( "should add a person with valid input through the API", async () => {
    ...
    await chai.request( server ).put( "/api/people" ).send( newPerson )
    const newPersonRow = await getPersonViaName( "TestName" )
    const checkPerson = newPersonRow[ 0 ]
  ...
  it( "should delete a person through the API", async() => {
    ...
    runhook = false
    const newPersonRow = await getPersonViaName( "TestName" )
    const checkPersonid = newPersonRow[ 0 ].id
