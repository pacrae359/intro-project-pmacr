...
/**
 * This functions is used to get a preexisting row in the people table of the database via an id.
...
/**
 * This functions is used to get a preexisting row in the people table of the database via a name.
 * @returns {Promise<object>}
 */
function getPersonViaName( name ) {
  return new Promise( ( res,rej )=>{
    // This runs the sql command against the database.
    db.all( "SELECT * FROM people WHERE name=?",[name],( err,row ) => {
      // If successful return the row, if not return the error.
      if( err ) {
        rej( err )
      } else {
